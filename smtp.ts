...
class GeminiService {
     ...
     */
    async cleanAndScrubText(rawText) {
     ...
     */
    async runGroundedQuery(query, referenceContext) {
        try {
            const response = await this.ai.models.generateContent({
                model: 'gemini-2.5-flash',
                contents: [
            ...
            throw new Error(`Failed to process query with Gemini grounding: ${error.message}`);
        }
    }
}
