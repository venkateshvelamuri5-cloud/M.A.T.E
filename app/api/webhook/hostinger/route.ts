  ...
  try {
    ...
    try {
      const { data: dbAgents } = await supabase
        .from('agents')
        .select('id, name, description, system_prompt, send_attachment');
      ...
      if (dbAgents && dbAgents.length > 0) {
        ...
        if (matchedAgent) {
          ...
          console.log(`Routed to Agent: ${matchedAgent.name}, send_attachment config: ${agentConfigSendAttachment}`);
        }
      }
    } catch (agentErr) {
